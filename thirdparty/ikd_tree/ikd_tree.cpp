...
int KD_TREE<PointType>::Delete_by_range(
  ...
  if (
    boxpoint.vertex_min[0] <= (*root)->node_range_x[0] && boxpoint.vertex_max[0] > (*root)->node_range_x[1] &&
  ...
  if (
    !(*root)->point_deleted && boxpoint.vertex_min[0] <= (*root)->point.x &&
...
void KD_TREE<PointType>::Delete_by_point(KD_TREE_NODE** root, PointType point, bool allow_rebuild) {
  ...
  if (
    ((*root)->division_axis == 0 && point.x < (*root)->point.x) ||
