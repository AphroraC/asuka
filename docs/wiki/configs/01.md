...
| 参数 | 作用 |
|---|---|
| `lidar_type: "livox"` | **雷达选择的唯一事实来源**：决定 `insert_frame` 的点云分发分支（Livox/Robosense），各算法也经 `active_lidar_key()` 以它选择 config_sensor 的段 |
...
| `map_saving_path` | 退出时落盘 `mapping_*.pcd` 的路径；同时作为扩展模块 `at_exit(map_saving_path)` 的入参。是否产出地图由所选算法的 odometry json（`mapping.enable_map_saving`，fastlio/batchlio/superlio）控制 |
...
跨文件耦合点：各算法经 `active_lidar_key()`（`core/sensor_config.hpp`）读取 config_ros 的 `lidar_type`，据此选择本文件中的段名——当前 `"livox"` 对应 `"livox"` 段，段缺失时算法构造期直接抛异常。换用 Robosense 需把 `lidar_type` 改为 `robosense`，并确保存在 `robosense` 段；未启用的段保留在文件中不会生效。
