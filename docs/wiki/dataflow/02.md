...
title: "线程模型与停机时序（风险点）"
description: >-
  单 worker 从 IMU/帧两条环形缓冲按到达序交错逐条消费，互斥锁加条件变量同步；stop 幂等可重入，save 前强制 join，析构按 worker → 扩展模块 → at_exit 排序；代码注释逐条说明规避数据竞争与销毁竞态的理由，是并发正确性的核心页面。
---

# 线程模型与停机时序（风险点）

本页聚焦 asuka 的并发骨架：数据在哪些线程之间流动、事件缓冲如何同步、停机为什么必须按特定顺序执行。核心结论先行：**`wait`/`wait_idle` 这类等待是"建议性"的，真正的并发正确性屏障是 `save()` 与析构路径里的 `stop()` + `join()`**。源码注释在两处（`asuka_ros.cpp` 的 `save()` 与 `~AsukaROS()`）逐条给出了这样设计的原始动机——一处规避对地图数据的数据竞争，一处规避模块销毁时的回调竞态。

涉及文件：

| 文件 | 职责 |
|---|---|
| `src/asuka/include/asuka/core/async_odometry_estimation.hpp` | 异步执行器的全部并发原语声明：线程、互斥锁、条件变量、三个原子标志、两条环形缓冲 |
| `src/asuka/src/asuka/core/async_odometry_estimation.cpp` | 消费循环、插入闸门、输入槽发射、幂等 `stop()` 的实现 |
...
| `AsyncOdometryEstimation::worker` | **唯一消费者**：按到达序逐条取走事件，发射输入槽后喂给 odometry 插件并排空计算 | 构造函数 `std::thread(&…::loop, this)` 启动，`stop()` 中 join |
| 扩展模块线程 | 各模块私有，由各自 `stop()` 管理 | 析构注释明确"then all module threads"，即所有模块线程必须在销毁前归零 |

构造顺序上有一个隐含保证：`asuka_ros.cpp` 构造函数先 `OdometryEstimation::load_module` 装载插件、再 `make_unique<AsyncOdometryEstimation>(odometry)`，因此 worker 启动时 `odometry` 共享指针必然已就绪，消费循环永远不会看到空算法。

---

## 二、生产者路径：insert 的锁与闸门

`insert_imu`（.cpp）与 `insert_frame` 结构完全一致：

```cpp
{
  std::lock_guard<std::mutex> lock(mtx);
  if (!running || end_of_sequence) return;   // 闸门：停机中一律丢弃
  imu_queue.push_back(imu);                  // 各流入各自的环形缓冲
}
...
3. **有界有损缓冲**：`boost::circular_buffer` 两条（`imu_queue_capacity = 1000`、`frame_queue_capacity = 50`），写满时**覆盖该流最旧事件**而非阻塞生产者。过载时丢旧数据，绝不反压 ROS 回调线程。入口的 ROS 订阅队列（rosnode 中 IMU 1000 / 点云 50）与之容量一一对应。

---

## 三、消费者路径：loop 的锁纪律

`loop()` 把每一轮拆成"锁内短临界区 + 锁外长计算"两段：

```cpp
// 锁内：等待 + 取出一条
cv.wait(lock, [this] { return !running || !imu_queue.empty() || !frame_queue.empty(); });
if (!running) break;
// 按队首传感器时间交错取出一条（某缓冲为空则取另一条）
// 解锁

// 锁外：发射输入槽 + 喂入 + 排空
processing = true;
if (event.imu) {
  Callbacks::on_insert_imu(event.imu);          // 输入槽：喂算法前统一发射
  odometry->insert_imu(event.imu);
} else {
  Callbacks::on_insert_frame(event.stamp, event.cloud);
  odometry->insert_frame(event.stamp, event.cloud);
}
...
- **锁内只取一条**：锁竞争被压到最小，同时按队首传感器时间交错取出保证投递顺序 = 到达顺序。
- **计算绝不持锁**：状态估计可能耗时数十毫秒以上，锁外执行保证生产者（ROS 回调）永远不被 worker 卡住——这对实时订阅是硬要求。
- **逐条投递**：复刻上游算法假设的 ROS 回调执行器语义——每条消息一次 insert + 一次排空。批量喂入或按类型重排会改变算法内部每一步的缓冲窗口，进而改变初始化窗口、关键帧边界与初始重力估计。
- **输入槽在框架层统一发射**：`on_insert_imu` / `on_insert_frame` 由执行器在喂算法之前发射，每条送达事件恰好一次（被队列丢弃的事件不会发射）——扩展观察到的输入流与所加载的前端算法无关；且三个链路槽（含 `on_new_frame`）同在 worker 线程，相对顺序即算法的程序序。
- **`process_once()` 排空循环**：一条数据入算法后可能产生多步内部计算，循环调用直到返回 false，保证条与条之间算法内部无残留负载。
...
| `mtx` + `cv` | mutex + condition_variable | 保护两条环形缓冲、唤醒 worker | 全体 |
| `imu_queue` / `frame_queue` | circular_buffer（1000 / 50） | 双输入缓冲，按传感器时间交错取出，满覆盖最旧 | 锁内读写 |
...
stateDiagram-v2
    ...
    R --> R: 逐条取事件 → 发射输入槽 → 喂算法 → process_once 排空
...
- `workload()`：锁内取 `queue.size()`，再加算法内部的 `odometry->workload()`。
...
3. **环形缓冲满覆盖最旧**：过载时最老 IMU/帧被**静默丢弃**（`insert_*` 返回值为 void，调用方无感知）。在线运行时通常意味着算力不足正在掉历史数据，而非报错。
