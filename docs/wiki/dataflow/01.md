...
title: "端到端数据流主链路"
description: >-
  ROS 消息 → 标定补偿与回环检查 → 算法私有 cloud_preprocess → 异步队列 → odometry 的 insert_imu/insert_frame 与 process_once 排空，完整链路贯穿四个层次；所有时间偏移与尺度修正收敛在 asuka_ros.cpp。
---
...
3. **异步队列层**（`core/async_odometry_estimation.cpp`）：双循环缓冲 + 单 worker 批量消费；
...
flowchart TD
    ...
    subgraph L3["③ 异步队列层 AsyncOdometryEstimation"]
        Q1["imu_queue circular_buffer(1000)"]
        Q2["frame_queue circular_buffer(50)"]
        W["worker::loop<br/>互斥锁+条件变量，成批取出"]
    end

    subgraph L4["④ 算法插件层 OdometryEstimation"]
        F1["odometry->insert_imu(imus)"]
...
- **CB1/CB2**：`ros::spin()` 的回调线程就是生产者线程；IMU 队列 1000、点云队列 50，与第三层循环缓冲容量一一对应（`asuka_rosnode.cpp#L19-L26`、`async_odometry_estimation.hpp#L28-L29`）。
...
- **W**：单 worker 成批搬空双队列后加锁释放，在锁外完成全部计算。
- **F1/F2 + D**：每批数据先全部喂 IMU 再喂帧（注释说明与 glim_ros 执行器顺序一致，`async_odometry_estimation.cpp#L75`），随后 `while(process_once())` 排空算法内部计算。
...
`AsyncOdometryEstimation` 用互斥锁 + 条件变量维护两个 `boost::circular_buffer`：

- `imu_queue` 容量 1000、`frame_queue` 容量 50（头文件 L28-L29 常量）；
- `insert_imu`/`insert_frame` 在锁内检查 `running && !end_of_sequence`，通过后 `push_back` 并 `cv.notify_one()`（cpp L17-L33）。stop 之后到达的数据被直接丢弃；缓冲满时由循环缓冲语义覆盖最旧数据；
- `loop()`（L59-L83）在锁内 `cv.wait` 直到有数据或被停止，随后**成批搬空双队列并解锁**，置 `processing = true`，按"先 IMU 后帧"的顺序喂给 odometry，再 `while (odometry->process_once()) {}` 排空计算，最后清 `processing`。

`workload()` 聚合两个队列长度与 `odometry->workload()`（L35-L38）；`asuka_rosnode` 退出前的 `wait(true)` 就是以 100Hz 轮询该值归零（`asuka_ros.cpp#L110-L116`）。
...
| `imu_queue` / `frame_queue` | circular_buffer 1000/50 | 输入缓冲，容量与 ROS 订阅队列对齐 |
...
- **回环只告警**：`handle_loop_back` 不做任何数据清理，回退数据仍会流入下游。
- **缓冲溢出**：`boost::circular_buffer` 满时 `push_back` 覆盖最旧数据，代码未显式告警。
