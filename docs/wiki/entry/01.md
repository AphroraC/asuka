...
AsukaROS 是一个以 **ROS 封装层为中枢、算法插件为外沿** 的激光-惯导里程计（LIO）框架。它的骨架可以概括为一句话：ROS 节点订阅 IMU 与点云两类话题，经标定补偿与回环检查后写入异步双队列；由独立 worker 线程成批投喂给通过 `so_name` 动态装载（dlopen）的五种里程计算法之一；配置系统、扩展模块与第三方库在其外围供装配。理解了这个中枢，整个仓库的其余页面都是围绕它的展开。
...
flowchart TB
    ...
    subgraph core["核心层 core/"]
        AE["AsyncOdometryEstimation<br/>imu_queue(1000) + frame_queue(50)<br/>单 worker：先 IMU 后帧 → process_once 排空"]
        OE["OdometryEstimation<br/>insert_imu / insert_frame / process_once /<br/>workload / stop / clear_buffers / save_map /<br/>cloud_preprocess / requires_imu 虚函数契约"]
    end
...
- **AsyncOdometryEstimation**：生产者-消费者枢纽，用互斥锁 + 条件变量维护双循环缓冲，把 ROS 回调线程与算法计算线程解耦。
...
2. **点云链**：`sensor_msgs::PointCloud2` → `AsukaROS::insert_frame`：按 `lidar_type` 分发 Livox / Robosense 两种自定义点 → `pcl::fromROSMsg` 转 PCL → `handle_loop_back(stamp, FrameId::LIDAR)` → `stamp += time_offset_lidar_to_imu` → 共享的 `cloud_preprocess->preprocess(...)`（**返回 null 则整帧丢弃**）→ `async->insert_frame` 入 `frame_queue`。
3. **消费链**：worker 线程在条件变量上等待 → 一次性搬空双队列并解锁 → **先喂 IMU 再喂帧**（注释明确说明对齐 glim_ros 执行器顺序）→ `while (odometry->process_once()) {}` 循环排空计算。
...
sequenceDiagram
    ...
    loop ros::spin()
        ...
        A->>Q: 回环检查 + 标定补偿后入队
        Q->>O: worker：先 IMU 后帧，process_once 排空
    end
...
- **AsyncOdometryEstimation 状态**：三个原子标志 `running`（初值 true）、`end_of_sequence`、`processing`，加上容量固定的双循环缓冲（`imu_queue_capacity = 1000`、`frame_queue_capacity = 50`）。
- **负载聚合**：`workload() = 双队列长度之和 + odometry->workload()`（算法内部负载）；`wait_idle()` 以 10ms 轮询等待三条件同时成立——无输入、无 processing、算法负载为 0。`wait(true)` 正是靠它支撑"队列清空再退出"。
...
4. **队列容量**：双队列是 boost `circular_buffer`，容量固定（1000/50），满载时按环形缓冲语义覆盖最旧数据。
