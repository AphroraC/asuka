...
title: "里程计抽象接口与插件契约"
description: >-
  OdometryEstimation 基类定义 insert_imu/insert_frame/process_once/workload/stop/clear_buffers/save_map/cloud_preprocess/requires_imu 等虚函数契约，static load_module 工厂与 extern C 的 create_odometry_estimation 构成动态插件边界，五种算法实现全部以此为接口面。
---
...
| 虚函数 | 默认实现 | 职责 | 典型调用方 |
|---|---|---|---|
| `insert_imu(const ImuData::ConstPtr&)` | 空操作 | 接收 IMU 数据 | 异步执行器 worker（先于帧） |
...
| `stop()` | 空操作 | 停止内部线程与计算 | `async->stop()` 链路、析构 |
| `clear_buffers()` | 空操作 | 清空内部缓冲 | 状态复位 |
| `save_map()` | 返回 `nullptr` | 导出累积地图 | save 流程（stop 之后） |
| `cloud_preprocess()` | 返回 `nullptr` | **反向交付**算法私有点云预处理器 | AsukaROS 构造期 |
| `requires_imu() const` | 返回 `true` | 能力声明：是否依赖 IMU | 上层能力查询 |
...
flowchart LR
    ...
    subgraph ASYNC["异步执行器"]
        AS["AsyncOdometryEstimation<br/>单 worker + 双队列"]
    end
    ...
    AS -- "insert_imu 先于 insert_frame" --> OD
...
2. 按契约覆写九个虚函数：数据入口 `insert_imu` / `insert_frame`、计算驱动 `process_once`、负载与停机 `workload` / `stop` / `clear_buffers`、导出 `save_map`、反向交付 `cloud_preprocess`、能力声明 `requires_imu`；
