...
| 槽 | 签名 | 载荷 | 语义（据命名与签名推断） |
|---|---|---|---|
| `on_insert_imu` | `void(const ImuData::ConstPtr&)` | IMU 样本（stamp / linear_acc / angular_vel） | IMU 数据进入主管线时 |
| `on_insert_frame` | `void(double, const PointCloudT::ConstPtr&)` | 时间戳 + `PointXYZIOffset` 点云 | 一帧点云进入主管线时 |
| `on_new_frame` | `void(const KeyFrame::ConstPtr&)` | 关键帧（位姿/速度/bias/去畸变点云） | 产生新关键帧 |
| `on_odometry_imu` | `void(const KeyFrame::ConstPtr&)` | 同上 | 里程计 IMU 预测后的状态 |
| `on_odometry_opt` | `void(const KeyFrame::ConstPtr&)` | 同上 | 里程计优化后的状态 |

所读证据中未包含发射点代码，上表语义列为推断；但 `callback_slot.hpp` 注释点名"发射线程例如 odometry worker"，且 `AsyncOdometryEstimation::loop` 正是独立 worker 中调用 `odometry->insert_imu/insert_frame/process_once` 的位置，可确认发射发生在数据热路径上。另注意 `KeyFrame::cloud_imu` 的注释——"每帧新分配、由 KeyFrame 持有，观察者可任意久持有"——这一所有权模型正是为回调观察者（如可视化、后端优化模块）设计的。
