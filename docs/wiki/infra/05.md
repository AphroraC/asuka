...
| 槽 | 签名（自 callbacks.hpp） | 事件语义 |
|---|---|---|
| `on_insert_imu` | `void(const ImuData::ConstPtr&)` | 执行器把 IMU 事件喂给算法前（worker 线程） |
| `on_insert_frame` | `void(double, const PointCloudT::ConstPtr&)` | 执行器把点云帧喂给算法前（worker 线程） |
| `on_new_frame` | `void(const KeyFrame::ConstPtr&)` | 前端算法产出新关键帧 |
| `on_odometry_imu` | `void(const KeyFrame::ConstPtr&)` | imu_prediction 的 IMU 前向传播结果 |
| `on_odometry_opt` | `void(const KeyFrame::ConstPtr&)` | 后端优化后的位姿结果 |

（各槽的发射者、线程与顺序契约已在 `callbacks.hpp` 的类注释中成文，详见"全局回调定义"页。）
