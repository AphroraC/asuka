...
1. **传感器外参**（`imu_preprocess.cpp`）：读 `config_sensor`，经 `active_lidar_key()` 按 `config_ros.lidar_type` 选择 `livox`/`robosense` 雷达块；对应块不存在则直接 `throw std::runtime_error`——**配置错误在进程启动即暴露，而不是在首帧数据时才失败**。外参缺省值为平移零、旋转单位阵，经 `SetExtrinsic` 注入。
...
flowchart TB
  ...
  CP -->|"PointCloudT 入异步环形缓冲"| ASYNC["AsyncOdometryEstimation<br/>单 worker 逐条消费"]
