...
4. **两级外参**：
   ...
   - `config_sensor` 中激活的 `livox`/`robosense` 块提供 `extrinsic_T/R`，构成雷达到 IMU 的 `Se3 lidar_imu`；两个块都不存在则直接抛异常。
5. **读 `config_ros`** 的 `enable_map_saving` 决定是否累积 `map_cloud`。
...
`insert_imu` / `insert_frame` / `workload` / `stop` / `clear_buffers` / `save_map` 全部以 `buffer_mutex` 保护，是异步执行器（AsyncOdometryEstimation）与算法之间的唯一并发接缝：

- `insert_imu`：持锁转发给 `imu_preprocess->insert_imu`，解锁后触发 `Callbacks::on_insert_imu`；
- `insert_frame`：用 `stamp + points.back().offset / 1000.0` 计算 `end_time`——即约定点云内点按时间序排列、`offset` 存毫秒级相对时间偏移；随后打包 `LidarData` 入 `lidar_buffer` 并触发 `on_insert_frame`；
- `workload()` 只返回 `lidar_buffer.size()`，上层据此判断积压；
- `stop()` 清空双缓冲并复位 `lidar_pushed`；`clear_buffers()` 额外复位预处理累加器与时间戳游标；
