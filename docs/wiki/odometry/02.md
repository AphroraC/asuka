...
- **结果输出**：每帧构造 `KeyFrame` 并通过全局回调 `Callbacks::on_new_frame` 发布；地图不常驻累积，`save_map()` 被调用时从 ikd-tree 现场导出。
...
2. **读取配置**：`config_odometry` 提供 `max_iteration`（默认 4）、`laser_point_cov`（0.001）、`fov_degree`、`filter_size_surf/map`、`cube_side_length`、`max_distance`（作为 `det_range`）、缓冲容量（lidar 50 / imu 1000）与 `mapping.enable_map_saving`（门控 `save_map()` 的地图导出）；
...
地图**不常驻累积**：`save_map()` 在被调用时（此时 worker 已被上层 stop/join）在 `buffer_mutex` 下 flatten ikd-tree、导出全部有效点为 `PointCloudT`，受 `mapping.enable_map_saving` 门控（关闭时返回空）。由于 `lasermap_fov_segment` 会裁剪远离视场的方块，导出内容是滑动局部地图——当前默认 `cube_side_length=1000` 时覆盖整条轨迹，调小 cube 后仅含当前窗口。安全依赖上层 `AsyncOdometryEstimation::save()` 先 join 工作线程的契约（与"地图保存与退出时序"页所述风险点呼应）。
