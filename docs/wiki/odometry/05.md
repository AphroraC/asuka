...
`asuka::lightning::OdometryEstimation` 是五种里程计插件中"第三方算法库 + asuka 框架"结合方式的典型样本：它自身不做滤波与地图算法，而是把 thirdparty/lightning 库的三件核心组件——`core/lio/eskf.hpp`（误差状态滤波）、`core/ivox3d/ivox3d.h`（三维体素局部地图）、`core/lio/imu_processing.hpp`（IMU 处理）——组装进 `asuka::OdometryEstimation` 插件契约（`insert_imu / insert_frame / process_once / workload / stop / clear_buffers / save_map / cloud_preprocess`）。
...
flowchart LR
  subgraph F["asuka 框架层"]
    ...
    CB["Callbacks<br/>on_insert_imu / on_insert_frame"]
...
图中关键点：实现类对 ESKF 的唯一耦合点是构造期注入的 `lidar_obs_func_` lambda（cpp L79-81），库的滤波框架因此保持通用；对 ivox 既是写入（地图增量）也是读取（近邻查询）；`Callbacks` 在两个入口尾部触发，使上层无需侵入算法即可挂钩。注意 `fasterlio::NUM_MAX_ITERATIONS` 等是**库级全局常量**，构造函数直接对其赋值——同类常量在进程内共享，多实例场景是潜在风险点。
...
4. 读 `config_sensor`：**必须**存在 `livox` 或 `robosense` 活动块，否则抛 `std::runtime_error`（插件装载即失败）；其 `extrinsic_R/T` 存为 `offset_r_lidar_fixed`/`offset_t_lidar_fixed`（lidar→IMU 外参），贯穿点云转世界与观测模型。
5. 配置 `ESKF::Options`（迭代上限、收敛阈值 `epsi_=1e-3`、`use_aa_`、观测回调）并 `kf.Init(...)`。

两个值得注意的细节：`keep_first_imu_estimation` 读入后立即 `(void)` 丢弃，是尚未生效的配置接缝；`stop()` 只做 `kill_switch = true` 加锁清空三缓冲（析构自动调用），`clear_buffers()` 额外把 `last_timestamp_imu` 复位为 -1。

## 数据入口与双 ESKF 状态设计

- **insert_imu**（L120-146）：asuka `ImuData` 转为 `::lightning::IMU`；锁内先做时间回环检测（`stamp < last_timestamp_imu` 则清空 imu_buffer 并告警），若 `imu_preprocess->is_initialized()` 则对 **kf_imu** 执行 `Predict(dt, q, gyro, acc)` 实时前向，最后入 `imu_buffer` 并触发 `Callbacks::on_insert_imu`。
- **insert_frame**（L148-161）：锁内做激光回环检测（清 lidar_buffer），对点云**深拷贝**后与时间戳分别入 `lidar_buffer`/`time_buffer`，触发 `on_insert_frame`。
...
| `last_timestamp_imu` / `last_timestamp_lidar` | `mtx_buffer` | 回环检测基准 |
...
- **时间回环**（rosbag 循环）：IMU 与激光分别检测并清空对应缓冲 + 告警。
- **退化输入**：点云 ≤1 点用均值扫描时长兜底；降采样后过少两档重试，仍 <5 点跳帧；有效面点 <20 时本次迭代放弃观测。
- **配置缺失**：`config_sensor` 无活动雷达块 → 构造抛异常。
...
- **入口回调**：`on_insert_imu` / `on_insert_frame` 已挂接，可无侵入添加监控。
