...
类声明 `class OdometryEstimation : public asuka::OdometryEstimation`（且显式 delete 拷贝），完整实现了基类契约：`insert_imu / insert_frame / process_once / workload / stop / save_map / cloud_preprocess`，其中 `cloud_preprocess()` 返回内部 `cloud_preprocess_impl` 供 ROS 层在入队前调用。

## 构造装配：三份配置 + 双滤波器

构造函数一次读齐三份配置：

- **config_odometry**：`batch`（dt=0.001、deskew、enable_openmp）、`mapping`（约 20 个键：imu_en、各类协方差、plane_thr、match_s、init_map_size、use_imu_as_input、gravity、enable_map_saving 等）、`odometry`（四个 circular_buffer 容量）、`filter`（surf/map 体素尺寸）；
- **config_sensor**：经 `active_lidar_key()` 按 `config_ros.lidar_type` 选择 `livox`/`robosense` 块，读取外参 `extrinsic_T/extrinsic_R`；**无对应雷达块直接 throw**，这是构造期唯一硬失败点。
...
| 分组 | 成员 | 说明 |
| --- | --- | --- |
| 输入缓冲 | `lidar_buffer`/`time_buffer`/`imu_buffer`（环形，容量可配）+ `last_imu_stamp` | 全部由 `buffer_mutex` 保护；环形写满覆盖最旧。`last_imu_stamp` 是纯水位（回环丢弃已移除，检测统一在 asuka_ros），供 synchronize 的数据饥饿判定使用 |
...
| 输出 | `map_cloud`、`pcd_save_en` | `save_map()` 直接返回 `map_cloud`（`mapping.enable_map_saving` 开启时逐帧累积） |

## 调用链：从 insert 到 process_once

上层 `AsyncOdometryEstimation` 按到达序逐条喂 `insert_imu/insert_frame`，每条之后循环调 `process_once()` 排空计算。输入槽 `on_insert_imu` / `on_insert_frame` 由执行器在喂算法之前统一发射，算法内不做回环丢弃（检测统一在 asuka_ros 告警）。主循环结构如下：

```mermaid
flowchart TB
    subgraph BUF["入口缓冲（buffer_mutex 保护）"]
        A["insert_imu：更新水位 → imu_buffer"]
        B["insert_frame：空帧检查 → lidar_buffer + time_buffer"]
    end
    A --> E
    B --> E
...
防御性边界汇总：空点云直接丢弃；IMU 未覆盖窗口尾则整帧等待；`config_sensor` 无 `lidar_type` 对应的雷达块抛异常；`ivox_nearby_type` 非法值回退；析构函数调 `stop()` 清空三个缓冲。时间回环不在算法内处理——`AsukaROS::handle_loop_back` 是唯一检测点（仅告警），`last_imu_stamp` 仅作同步水位使用。
