...
- **输入 1**：异步执行器在 worker 线程取出 IMU 事件、喂给算法之前发射 `on_insert_imu` → `ImuPrediction::on_insert_imu` 入队；
- **输入 2（锚点）**：里程计前端产出关键帧 → `on_new_frame` → `on_new_frame` 入队；
- **输出**：积分结果 → `emit_predicted_odometry` → `on_odometry_imu` 广播给订阅者。

三个链路槽共用前端 worker 线程，IMU 样本与锚点关键帧按算法程序序抵达模块的队列。
...
```mermaid
flowchart TD
    A["异步执行器 worker<br/>（喂算法前发射）"] -->|on_insert_imu| Q
