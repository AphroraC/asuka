...
| 异步执行器 | `core/async_odometry_estimation.cpp` | 前端 worker：按到达序逐条取出事件 -> 发射输入槽 -> `insert_imu` / `insert_frame` -> `process_once` 排空 |
| 前端插件 | `odometry/*/` | 仅前端 worker 访问算法状态（缓冲入队加锁） |
| 扩展 | `modules/*.cpp` | 各自线程（imu_prediction / optimization_* 为"回调只入队"模式） |

`AsyncOdometryEstimation`：IMU 与帧各一条有界环形缓冲（1000 / 50，满则丢该流最旧），
worker 按队首传感器时间交错逐条取出（即到达序）、**每条先发射 `on_insert_imu` /
`on_insert_frame` 再喂给算法**，随后排空 `process_once()`；`stop()` 置位并 join worker。
...
每个槽的**发射者、线程、顺序与丢弃语义**以 `core/callbacks.hpp` 的注释为准：输入槽
（`on_insert_imu` / `on_insert_frame`）由 `AsyncOdometryEstimation` 在 worker 线程上、
喂算法之前统一发射——扩展观察到的输入流与所加载的前端算法无关；`on_new_frame` 由
各前端算法在 worker 线程发射；`on_odometry_imu` / `on_odometry_opt` 分别由
imu_prediction 与后端优化扩展在自己的线程上发射。
...
| `lidar_type` 是雷达选择的唯一事实来源 | config_sensor 的块选择由 `active_lidar_key()` 依 `config_ros.lidar_type` 解析，两处不会错配 |
| 输入槽发射在执行器、输出槽发射在前端/扩展 | 框架保证"喂给算法的每条数据同时可见于订阅者"；重活由扩展自行入队转移 |
